...
// Strip inline comments (everything after ';' or '#' is dropped, except inside
// quotes -- simplified here for now).
...
bool IniConfig::Load(const std::string& path) {
    ...
    std::string line;
// 空串表示全局段
// empty string = global section
    while (std::getline(in, line)) {
        ge::StripUtf8Bom(line);
        line = Trim(StripComment(line));
        if (line.empty()) continue;
// 容错：文件带 BOM 时剥除，不带时无影响
// fault-tolerance: strip a leading BOM if present, no-op otherwise
...
int IniConfig::GetInt(const std::string& section,
    ...
    try {
        ...
        // 支持 0x 前缀的十六进制
        // support 0x-prefixed hexadecimal
        return def;
    }
}
